...
if (isset($_GET['language']) && in_array($_GET['language'], ['fr', 'en'])) {
    ...
    $redirectUrl = 'install_corrected.php';
    if ($currentStep > 1) {
...
if (isLaravelInstalled() && !isset($_GET['force'])) {
    showError('Application déjà installée' . ' <a href="install_corrected.php?force=1" style="color: #007bff;">Forcer la réinstallation</a>');
    exit;
}
        ...
        switch ($step) {
            case 1: // Vérification de la licence
                if (empty($_POST['serial_key'])) {
                    ...
                    writeToLog("Clé de licence mémorisée en session: " . $licenseKey, 'SUCCESS');
                    ...
                    if (!$licenseCheck['valide']) {
                        ...
                        if ($isAjax) {
                            ...
                            echo json_encode([
                                ...
                                'redirect' => 'install_corrected.php?step=2'
                            ]);
                            exit;
                        }
            ...
            case 2: // Vérification des prérequis système (passage automatique)
                ...
                if (!$canContinue) {
                    ...
                    if (!$isAjax) {
                        header('Location: install_corrected.php?step=3');
                        exit;
                    }
            ...
            case 3: // Configuration de la base de données
                ...
                if (!empty($missingFields)) {
                    ...
                    try {
                        ...
                        if (!$isAjax) {
                            header('Location: install_corrected.php?step=4');
                            exit;
                        }
            ...
            case 5: // Installation finale
                try {
                    ...
                    header('Location: install_corrected.php?success=1');
                    exit;
                } catch (Exception $e) {
