...
@section('title', 'Modifier Section À propos')
    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.cms.about.update', $about) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <textarea class="form-control @error('content') is-invalid @enderror" 
                                          id="content" name="content" rows="8" required>{{ old('content', $about->content) }}</textarea>
                                @error('content')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-control @error('section_type') is-invalid @enderror" 
                                    ...
                                    <option value="text" {{ old('section_type', $about->section_type) == 'text' ? 'selected' : '' }}>Texte simple</option>
                                    <option value="image_left" {{ old('section_type', $about->section_type) == 'image_left' ? 'selected' : '' }}>Image à gauche</option>
                                    <option value="image_right" {{ old('section_type', $about->section_type) == 'image_right' ? 'selected' : '' }}>Image à droite</option>
                                    <option value="full_width" {{ old('section_type', $about->section_type) == 'full_width' ? 'selected' : '' }}>Pleine largeur</option>
                                </select>
                                @error('section_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" class="form-control @error('sort_order') is-invalid @enderror" 
                                       id="sort_order" name="sort_order" value="{{ old('sort_order', $about->sort_order) }}" min="0" required>
                                @error('sort_order')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="text-end">
                            <a href="{{ route('admin.cms.about.show', $about) }}" class="btn btn-light me-2">Annuler</a>
