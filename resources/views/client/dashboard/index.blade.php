...
<body class="bg-light">
    ...
    <div class="sidebar text-white">
        ...
        <nav class="mt-4">
            <ul class="nav nav-pills flex-column">
                <li class="nav-item">
                    <a class="nav-link active text-white" href="{{ route('client.dashboard') }}">
                        <i class="fas fa-tachometer-alt me-2"></i> Dashboard
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link text-white-50" href="{{ route('client.projects.index') }}">
                        <i class="fas fa-folder me-2"></i> Projets
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link text-white-50" href="{{ route('client.licenses.index') }}">
                        <i class="fas fa-key me-2"></i> Licences
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link text-white-50" href="{{ route('client.support.index') }}">
                        <i class="fas fa-life-ring me-2"></i> Support
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link text-white-50" href="{{ route('subscription.plans') }}">
                        <i class="fas fa-credit-card me-2"></i> Abonnement
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link text-white-50" href="{{ route('client.settings.index') }}">
        ...
        <div class="position-absolute bottom-0 w-100 p-4">
            <div class="d-flex align-items-center">
                ...
                <div class="flex-grow-1">
                    ...
                    <small class="text-white-50">{{ $client->company_name }}</small>
                </div>
        ...
        <div class="container-fluid p-4">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <p class="text-muted">Bienvenue {{ $client->name }}, voici un aperçu de votre compte</p>
                </div>
                @if($subscription)
                    <div class="text-end">
                        <span class="badge bg-{{ $subscription->status === 'active' ? 'success' : ($subscription->status === 'trial' ? 'info' : 'warning') }} me-2">
                            {{ ucfirst($subscription->status) }}
                        </span>
                        <small class="text-muted">{{ $subscription->plan->name ?? 'Plan inconnu' }}</small>
                    </div>
                @endif
            </div>
                ...
                <div class="row mb-4">
                    <div class="col-12">
                        @foreach($notifications as $notification)
                            <div class="alert notification-card notification-{{ $notification['type'] }} d-flex align-items-center mb-3">
                                ...
                                @if(isset($notification['action']))
                                    <a href="{{ $notification['action']['url'] }}" class="btn btn-sm btn-outline-primary">
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    ...
                                    <h3 class="mb-0">{{ $usageStats['projects']['count'] }}</h3>
                                    @if($usageStats['projects']['limit'] > 0)
                                        <small class="text-muted">/ {{ $usageStats['projects']['limit'] }}</small>
                                    @endif
                                </div>
                            ...
                            @if($usageStats['projects']['limit'] > 0)
                                <div class="mt-3">
                                    <div class="progress" style="height: 4px;">
                                        <div class="progress-bar" style="width: {{ $usageStats['projects']['percentage'] }}%"></div>
                                    </div>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    ...
                                    <h3 class="mb-0">{{ $usageStats['licenses']['count'] }}</h3>
                                    @if($usageStats['licenses']['limit'] > 0)
                                        <small class="text-muted">/ {{ $usageStats['licenses']['limit'] }}</small>
                                    @endif
                                </div>
                                <div class="progress-circle" style="background: linear-gradient(45deg, #1cc88a, #13855c);">
                                    {{ $usageStats['licenses']['percentage'] }}%
                                </div>
                            </div>
                            @if($usageStats['licenses']['limit'] > 0)
                                <div class="mt-3">
                                    <div class="progress" style="height: 4px;">
                                        <div class="progress-bar bg-success" style="width: {{ $usageStats['licenses']['percentage'] }}%"></div>
                                    </div>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    ...
                                    <h3 class="mb-0">{{ $usageStats['licenses']['active'] }}</h3>
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    ...
                                    <h3 class="mb-0">{{ $usageStats['support_tickets']['open'] }}</h3>
                        ...
                        <div class="card-body">
                            ...
                            @empty
                                <div class="text-center text-muted py-4">
                                    ...
                                    <a href="{{ route('client.projects.create') }}" class="btn btn-primary btn-sm mt-2">
                                        Créer un projet
                                    </a>
                                </div>
                            @endforelse
                ...
                <div class="col-12">
                    <div class="card shadow">
                        <div class="card-header py-3 d-flex justify-content-between align-items-center">
                            ...
                            <a href="{{ route('client.licenses.index') }}" class="btn btn-sm btn-primary">
                        ...
                        <div class="card-body">
                            @forelse($recentLicenses as $license)
                                <div class="row align-items-center mb-3 pb-3 {{ !$loop->last ? 'border-bottom' : '' }}">
                                    <div class="col-md-4">
                                        <h6 class="mb-0">{{ $license->serial_key }}</h6>
                                        <small class="text-muted">{{ $license->project->name ?? 'Projet supprimé' }}</small>
                                    </div>
                                    <div class="col-md-2">
                                    ...
                                    <div class="col-md-2">
                                        <small class="text-muted">
                                            @if($license->expires_at)
                                                Exp. {{ $license->expires_at->format('d/m/Y') }}
                                            @else
                                                Permanent
                                            @endif
                                        </small>
                                    ...
                                    <div class="col-md-2 text-end">
                                        <a href="{{ route('client.licenses.show', $license) }}" class="btn btn-sm btn-outline-primary">
                                            Détails
                                        </a>
                                    </div>
                                </div>
                            @empty
                                <div class="text-center text-muted py-4">
                                    ...
                                    <a href="{{ route('client.licenses.create') }}" class="btn btn-primary btn-sm mt-2">
                                        Créer une licence
                                    </a>
                                </div>
                            @endforelse
    ...
    <script>
        ...
        function loadActivityChart(days = 30) {
            fetch(`{{ route('client.dashboard') }}/activity-chart?days=${days}`)
